...
/**
 * Implements a predictor selectivo FAN(classificator with continuous class 
 * variable) where all the variables, including the class, are of class MTE.
...
public class SelectiveFANMTEPredictor extends FANMTEPredictor{
     
    double rmse_mean_M, rmse_median_M, lcc_mean_M,lcc_median_M;
    
    public SelectiveFANMTEPredictor() {
   ...
   /**
     * Creates a SelectiveFANMTEPredictor from a database. Feature subset 
     ...
     */
    public SelectiveFANMTEPredictor(DataBaseCases dbTrain, DataBaseCases dbTest, int cv, int intervals) throws ParseException,IOException{

             
        FANMTEPredictor M, M1;
...
*/
        ...
        int nlinks=(netNodeList.size()-1)/2;
        //Create the initial NaiveBayes Model
        M=new FANMTEPredictor(dbTrainAux, 0, intervals,nlinks);
        ...
        for (int i=1;i<v_OrderedMI.size()-1;i++){
            ...
            FANMTEPredictor Maux=new FANMTEPredictor();
           // FANMTEPredictor Maux=new FANMTEPredictor(dbTrainAux,0,intervals);
            Maux=copy_model(M);
...
private Vector OrderedIndexByMutualInformation (Vector W){
   ...
   /**
     * Main for constructing a MTE selective FAN  from a data base.
     * Arguments:
    ...
    public static void main(String args[]) throws ParseException,IOException {
         ...
         if (args[1].compareTo("CV") == 0) { // Cross validation
            ...
            for (i=0 ; i<k ; i++) {
                ...
                SelectiveFANMTEPredictor pred = new SelectiveFANMTEPredictor(dbTrain, dbTest,classv,interv);              
            ...
            SelectiveFANMTEPredictor pred = new SelectiveFANMTEPredictor(cTrain, cTest, classv,interv);
            pred.saveNetwork("SelectiveFAN.elv");
        }
