...
public abstract class Learning  {
   ...
   public LinkList[] compareOutput(Bnet b){
	...
	for(i=0 ; i< linksbcopy.size(); i++)
	    ...
	    nodeT =(Node)link.getTail();
	    nodeH =(Node)link.getHead();
            System.out.println("Checking for " + nodeT.toString() + " to " + nodeH.toString());System.out.println("Checking for original order tail to head");
	    linkbap = bapcopy.getLink(nodeT,nodeH);System.out.println("Now checking for head to tail");
	    linkbap2 = bapcopy.getLink(nodeH,nodeT);
	    if ((linkbap != null) || (linkbap2 != null)){
	       if (linkbap != null){ System.out.println("Found link Tail to Head");
	          if (link.getDirected()){
                      System.out.println("Link is directed in original model");
		     if (!linkbap.getDirected()){ linkschange.insertLink(link);System.out.println("Not directed in learned");
                     }else
		  ...
		  {if (linkbap2.getDirected()) linkschange.insertLink(link);
           ...
           } else {linksdel.insertLink(link); System.out.println("Deleted > " + link.toString());}
	   }
	...
	for(i=0 ; i< linksbapcopy.size();i++)
	   {
	    ...
	    if ((link == null) && (link2 == null)){ linksadd.insertLink(linkbap); System.out.println("Added> " + linkbap.toString());}
	   }
System.out.println("Links changed > " + linkschange.toString());
	addel[0]=linksadd;
	addel[1]=linksdel;
	addel[2]=linkschange;
	return addel;
