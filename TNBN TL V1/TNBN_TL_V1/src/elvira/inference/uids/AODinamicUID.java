...
/*		protected NodeAODinamicUID selectCandidate(ArrayList<NodeAOUID> candidates) {
	...
	protected void applyDinamicProgrammingAtTheEndOfGSDAG() {
		...
		if ((parents!=null)&&(parents.size()==1)){
			applyDinamicProgrammingUntilTheFirstBranch(tree.getGsdag().getLastNodeGSDAG(), 2);
		}
		else{
			//We do nothing
			return;
	 ...
	 */
	protected void applyDinamicProgrammingUntilTheFirstBranch(NodeGSDAG start,int maxNumNodesGSDAG) {
		// TODO Auto-generated method stub
		...
		if (isBranch){
			collectRelationsInBranch(currentNode);
		}
	}
	...
	protected void applyDinamicProgrammingLimited(NodeGSDAG start,int maxNumVariables) {
		...
		if (start!=null){
			...
			//time that we eliminate variables in the node
				...
				collectedInBranch = collectRelationsInBranch(currentNode);
