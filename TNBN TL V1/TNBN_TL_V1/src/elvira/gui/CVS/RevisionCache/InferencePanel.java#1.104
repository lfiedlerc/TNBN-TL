      ...
      if (e.isPopupTrigger()) {
         ...
         if (currentNode != null) {
           ...
           if (currentNode.getClass()==Continuous.class){
            ...
            if (this.getBayesNet().getClass()==IDWithSVNodes.class){
            	...
            	showDecisionTableMenuItem.setText(localize(menuBundle,"Popup.ShowDecisionTables.label"));
           		...
           		showDecisionTableMenuItem.setEnabled((currentNode.getKindOfNode()==Node.DECISION)&&(!((IDiagram)getBayesNet()).hasForcedPolicy(currentNode)));
           		//For chance nodes
           		showPosteriorDistributionsTableMenuItem.setText(localize(menuBundle,"Popup.ShowPosteriorDistributionsTables.label"));
           		inferencePopupMenu.insert(showPosteriorDistributionsTableMenuItem,1);
           		showUtilitiesTableMenuItem.setText(localize(menuBundle,"Popup.ShowUtilitiesTables.label"));
           		inferencePopupMenu.insert(showUtilitiesTableMenuItem,2);
     ...
     for (e = bayesNet.getNodeList().elements(); e.hasMoreElements(); ) {
		...
		if (node.getPosX() >= 0) {
	  	  boolean enter = true;
                  ...
                  FontMetrics fm=getFontMetrics(getFont(node.getFont()));
                  Color colornode = NODE_COLOR;
                  


/*mluque*/	if ((comparingcase!=null) && (node.getClass()!=Continuous.class)) {
...
///*andrew*/   	else if (node.getClass() == Continuous.class) 
/*mluque*/      else if ((node.getClass() == Continuous.class) && (node.getKindOfNode()!=Node.UTILITY)&&(node.getKindOfNode()!=Node.SUPER_VALUE))
