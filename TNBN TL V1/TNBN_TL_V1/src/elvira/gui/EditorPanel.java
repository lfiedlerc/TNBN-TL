...
public class EditorPanel extends ElviraPanel implements MouseListener, MouseMotionListener {
   ...
   public static final String[] functionsNode={"Treatment", "Riskfactor", "Symptom", "Sign", "Test", "Disease", "Aux", "Other", "Defined", ""};
...
/*	bayesNetClass = bayesNet.getClass();
	...
	if (IDWithSVNodes.class.isInstance(bayesNet)&&showInfluences){
			IDWithSVNodes auxID;
			auxID = (IDWithSVNodes)bayesNet;
			boolean propagate;
			if (auxID.getCpn()==null){
	dsdfsf			
        		if (auxID.hasOnlyOneTerminalValueNode()==false){
        			propagate = askWhetherAddATerminalValueNodeInfluences(auxID);
        		}
        		else{
        			propagate = true;
        		}
        		...
        		if (propagate){			
			 	...
			 	CooperPolicyNetwork cooperPN = CooperPolicyNetwork.constructCPNFrom(auxID,3,null);
             	//The cooper policy network must also be compiled.
             	cooperPN.compile(0,null,null,null);
             	
             	auxID.setCpn(cooperPN);
        		}
        		else{
...
//    if (link.getColorLink()==null){
		...
		if (((bayesNetClass==Bnet.class)||(bayesNetClass==IDiagram.class)||(bayesNetClass==IDWithSVNodes.class))
    			&&showInfluences){
   ...
   public void createNode(int x, int y) {
      ...
      switch (nodeType) {
         ...
         case Node.UTILITY: nodeName = bayesNet.generateSpecialName("U",utilityNodes);
                 ...
                 break;
         case Node.OBSERVED:
        	 	nodeName = bayesNet.generateSpecialName("O",observedNodes);
        	 	observedNodes++;
        	 	break;
