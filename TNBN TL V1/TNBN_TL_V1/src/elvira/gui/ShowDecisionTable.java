...
public class ShowDecisionTable extends JDialog {
  ...
  private Vector potentVectorForPolicies;
 ...
 public ShowDecisionTable(Node node, Bnet bnet, int indexMethod) {
	...
	if (((Node) ((Potential) potVector.elementAt(i)).getVariables().elementAt(((Potential) potVector.elementAt(i)).getVariables().size()-1)).getName().equals(node.getName())) {
...
/*	if ((indexMethod>=0)&&(indexMethod<=5)){
		 potVector=bnet.getCompiledPotentialList();
		 potVectorForPolicies=potVector;
	 }
	 else{
  ...
  private void generateDecisionTableDecisions(Node node, Potential pot) {
