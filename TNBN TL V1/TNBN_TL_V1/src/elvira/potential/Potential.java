...
public abstract class Potential implements Cloneable, Serializable{
  ...
  public void save(PrintWriter p){
    System.out.println("Error in Potential.save(PrintWriter): this method must be overloaded in "+
            this.getClass());
    System.exit(1);
  }
  ...
  public void normalize(){
    System.out.println("Error in Potential.normalize(): this method must be overloaded in "+
            this.getClass());
    System.exit(1);
  }
  ...
  public double getValue(Configuration conf){
    System.out.println("Error in Potential.getValue(Configuration): this method must be overloaded in "+
            this.getClass());
    System.exit(1);
    return 0.0;
  }
  ...
  public void setValue(Configuration conf, double val){
    System.out.println("Error in Potential.setValue(Configuration,double): this method must be overloaded in "+
            this.getClass());
    System.exit(1);
  }
  ...
  public long getSize(){
    System.out.println("Error in Potential.getSize(): this method must be overloaded in "+
            this.getClass());
    System.exit(1);
    return 0;
  }
  ...
  public double totalPotential(){
    System.out.println("Error in Potential.totalPotential(): this method must be overloaded in "+
            this.getClass());
    System.exit(1);
    return 0;
  }
  ...
  public double totalPotential(Configuration conf){
    System.out.println("Error in Potential.totalPotential(Configuration): this method must be overloaded in "+
            this.getClass());
    System.exit(1);
    return 0;
  }
  ...
  public double entropyPotential(){
    System.out.println("Error in Potential.entropyPotential(): this method must be overloaded in "+
            this.getClass());
    System.exit(1);
    return 0;
  }
  ...
  public double entropyPotential(Configuration conf){
    System.out.println("Error in Potential.entropyPotential(Configuration): this method must be overloaded in "+
            this.getClass());
    System.exit(1);
    return 0;
  }
  ...
  public Potential marginalizePotential(Vector vars){
    System.out.println("Error in Potential.marginalizePotential(Vector): this method must be overloaded in "+
            this.getClass());
    System.exit(1);
    return null;
  }
   ...
   * @return a <code>Vector</code> of objects of class <code>Node</code>
  ...
  /**
   * This method orders the potential making the order of the variables is consistent
   * with 'order'. Not all the variables of the potential have to be elements of 'order'.
   * This method is used to present the policies of the ID with the variables in temporal order.
   * @param x the <code>Node</code> to be the last.
   */
   ...
   * (At the moment the method is overwriten only in
  ...
  public void showResult() {
    ...
    System.out.println("At the moment the method (Potential)showResults is not implemented for "
    + this.getClass().getName());
    /*andrew*/ print();
    // System.exit(0);
  }
  ...
  /**
   * Append a new transparent variable to this Potential with <code>npoints</code> cases
   ...
   * @return a Vector with the list of transparent variables from this potential
   */
   ...
   * @return a Vector with the list of non transparent variables from this potential
   */
   ...
   * are related to a given configuration. Among the variables of the configuration
   * (first argument) there will be a special one. From the given configuration,
  ...
  /**
   * Method to get a set of values from a potential. The values to get are related
   * to a given configuration, but will consider all the values for a special
   * variable passed as second argument. This method is used to the the utilities
   * related to a given configuration, and for all the alternatives for that
   * decision
   * @param configuration base Configuration 
   * @param variable special variable. Must be contained in the variables of
   *        the potential
 ...
 /**
  * Method to set values in the potential. The values will be stored in a configuration
  * composed with the configuration passed as argument and with another variables passed
  * as a second argument. The rest of arguments will be a vector of repetitions (only if
  * this vector is not null it will be needed to store several values in the potential) 
  * and the value to store
  * @param baseConf where to store the value
  * @param variable to add to the variables in configuration. So it is completely
  *        defined the configuration where the value will be stored
  * @param valForVariable to assign to the variable, to complete the configuration where 
  * to store a value
  * @param repetitions of repetitions. If this vector is not null several positions in the
  *        potential will be assigned the same value 
