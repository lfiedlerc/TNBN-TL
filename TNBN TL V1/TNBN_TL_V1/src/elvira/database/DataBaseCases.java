...
public class DataBaseCases extends Bnet implements ConditionalIndependence,
    ...
    public void saveDataBase(FileWriter f) throws IOException {
     ...
     public void saveDataBaseFiniteStates(FileWriter f) throws IOException {
        int i, j;
        Bnet network = (Bnet) this;
        NodeList variables;
        PrintWriter fv = new PrintWriter(f);
        ...
        for(int in=0;in<variables.size();in++){
            Node nod =variables.elementAt(in);
            nod.setTypeOfVariable(Node.FINITE_STATES);
        }
        variables.save(fv);
     ...
     public void saveDataBaseSimple(FileWriter f) throws IOException {
        int i, j;
        Bnet network = (Bnet) this;
        NodeList variables;
        PrintWriter fv = new PrintWriter(f);
   //     NodeList nodes = this.getVariables();
        ...
        Relation relation = (Relation) vector.elementAt(0);
        CaseList caselistmem = (CaseList) relation.getValues();
        CaseListMem listaSimple= (CaseListMem) relation.getValues();
        int n = caselistmem.getNumberOfCases();
        ...
        for (i = 0; i < caselistmem.getNumberOfCases(); i++) {
            fv.print("[");
            for (int k=0;k<listaSimple.getCase(0).length;k++){
                    fv.print(listaSimple.getCase(i)[k]);
                    if (k+1!=listaSimple.getCase(0).length)
                         fv.print(",");
            }
            fv.print("]\n");
        }
